...
const order = inventors.sort((firtsperson, lastperson) => {
  if (firtsperson.year > lastperson.year) {
    return 1;
  } else {
    return -1;
  }
...
const order2 = inventors.sort((a, b) => (a.year > b.year ? 1 : -1));
...
const orderPeople = people.sort((a, b) => {
...
const trasport = data.reduce((obj, item) => {
  if (!obj[item]) {
    obj[item] = 0;
  }
  obj[item]++;
